...
2. Upload or push the contents of this `space_debris_streamlit` folder. Make sure `app.py`, `requirements.txt`, and `runtime.txt` are visible in the repository root.
...
If Streamlit Cloud says `ModuleNotFoundError` for `plotly`, `pandas`, `sklearn`, or `joblib`, your `requirements.txt` is not in the same GitHub folder level as `app.py`. Move it to the root of the repository and reboot the app from **Manage app**.
